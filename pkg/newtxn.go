...
type txState struct {
	...
	acc           *Account    // account reference for Private Key (to sign tx)
...
func CreateTxn(payTo []PayTo, fixedFee CoinAmount, acc Account, store Store, lib L1) (NewTxn, error) {
	...
	state := &txState{
		...
		outputSum:     outputSum,
		used:          NewUTXOSet(),
		source:        NewUTXOSource(store, acc.Address),
...
func calculateFee(fixedFee CoinAmount, state *txState) (NewTxn, error) {
	...
	for {
		...
		newTx, err := state.lib.MakeTransaction(state.inputs, state.outputs, fee, state.changeAddress, state.acc.Privkey)
		//txHex, err := buildTxn(state)
		if err != nil {
			return NewTxn{}, err
		}
